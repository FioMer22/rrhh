...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$titulo || !$cuerpo) {
        ...
        if ($destinatario === 'todos') {
            ...
            // Solo admins y rrhh
            $st = $pdo->query("
                SELECT DISTINCT ur.usuario_id AS id FROM usuarios_roles ur
...
$totalSubs = (int)$pdo->query("SELECT COUNT(DISTINCT usuario_id) FROM push_suscripciones")->fetchColumn();
...
<head>
    ...
    <style>
        ...
        .dest-btn {
            padding: 8px 16px; border-radius: 10px; border: 2px solid #e5e7eb;
            cursor: pointer; font-size: 14px; background: #fff; transition: all .15s;
        }
        .dest-btn.active { border-color: #3b82f6; background: #eff6ff; color: #1d4ed8; font-weight: 600; }
        .stat-chip {
...
<div class="wrap" style="max-width:700px">
    <div style="margin-bottom:16px">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px">
            <span class="stat-chip">📱 <?= $totalSubs ?> dispositivos suscritos</span>
            <span class="stat-chip">👥 <?= $totalUsuarios ?> empleados activos</span>
        </div>
    </div>

    <?php if ($ok): ?>
        <div class="alert alert-ok" style="margin-bottom:14px"><?= e($ok) ?></div>
    <?php endif; ?>
    <?php if ($err): ?>
        <div class="alert alert-err" style="margin-bottom:14px"><?= e($err) ?></div>
    <?php endif; ?>
    ...
    <div class="card form-card">
        <form method="post" id="formNotif">
            ...
            <div style="margin-top:16px;display:flex;gap:10px">
                <button type="submit" style="background:#0b1f3a">
                    📣 Enviar notificación
                </button>
